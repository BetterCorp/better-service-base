...
exports.default = (genNewPlugin, maxTimeoutToExpectAResponse) => describe('EmitAndReturn', async () => {
  ...
  describe('emitEventAndReturn', async () => {
    ...
    it('should be able to emit to events with plugin name defined', async () => {
      ...
      emitter.onReturnableEvent('caller', 'plugin', 'event', (resolve, reject, data) => {
        assert.ok(true, 'Received onEvent');
        resolve(emitData2);
      });
      const resp = await emitter.emitEventAndReturn('caller', 'plugin', 'event', {}, maxTimeoutToExpectAResponse / 1000);
    ...
    it('should be able to emit to events with self', async () => {
      ...
      emitter.onReturnableEvent('caller', null, 'event', (resolve, reject, data) => {
        assert.ok(true, 'Received onEvent');
        resolve(emitData2);
      });
      const resp = await emitter.emitEventAndReturn('caller', null, 'event', emitData, maxTimeoutToExpectAResponse / 1000);
      clearTimeout(emitTimeout);
      assert.ok(true, 'Received Response');
    });
    it('should not be able to emit to other events with plugin name defined', async () => {
      ...
      try {
        await emitter.emitEventAndReturn('caller', 'plugin', 'event2', emitData, maxTimeoutToExpectAResponse / 1000);
        clearTimeout(emitTimeout);
    ...
    it('should not be able to emit to other events with self', async () => {
      ...
      try {
        await emitter.emitEventAndReturn('caller', null, 'event2', emitData, maxTimeoutToExpectAResponse / 1000);
        clearTimeout(emitTimeout);
        assert.fail('EEAR Returned');
      } catch (exc) {
        clearTimeout(emitTimeout);
    ...
    it('should timeout correctly', async () => {
      ...
      try {
        await emitter.emitEventAndReturn('caller', null, 'event', emitData, maxTimeoutToExpectAResponse / 1000);
        clearTimeout(emitTimeout);
        assert.fail('EEAR Returned');
      } catch (exc) {
        clearTimeout(emitTimeout);
  ...
  for (let typeToTest of typesToTest) {
    describe(`emitEventAndReturn ${typeToTest.name}`, async () => {
      it('should be able to emit to events with plugin name defined', async () => {
        ...
        const resp = await emitter.emitEventAndReturn('caller', 'plugin', 'event', typeToTest.data, maxTimeoutToExpectAResponse / 1000);
        clearTimeout(emitTimeout);
      ...
      it('should be able to emit to events with self', async () => {
        ...
        const resp = await emitter.emitEventAndReturn('caller', null, 'event', typeToTest.data, maxTimeoutToExpectAResponse / 1000);
        clearTimeout(emitTimeout);
      ...
      it('should not be able to emit to other events with plugin name defined', async () => {
        ...
        try {
          await emitter.emitEventAndReturn('caller', 'plugin', 'event2', typeToTest.data, maxTimeoutToExpectAResponse / 1000);
          clearTimeout(emitTimeout);
          assert.fail('EEAR Returned');
        } catch (exc) {
          clearTimeout(emitTimeout);
      ...
      it('should not be able to emit to other events with self', async () => {
        ...
        try {
          await emitter.emitEventAndReturn('caller', null, 'event2', typeToTest.data, maxTimeoutToExpectAResponse / 1000);
          clearTimeout(emitTimeout);
          assert.fail('EEAR Returned');
        } catch (exc) {
          clearTimeout(emitTimeout);
      ...
      it('should timeout correctly', async () => {
        ...
        try {
          await emitter.emitEventAndReturn('caller', null, 'event', typeToTest.data, maxTimeoutToExpectAResponse / 1000);
          clearTimeout(emitTimeout);
          assert.fail('EEAR Returned');
        } catch (exc) {
          clearTimeout(emitTimeout);
