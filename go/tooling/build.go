...
func SyncClients(cwd string) ([]string, error) {
	...
	existing, err := filepath.Glob(filepath.Join(output, "*.go"))
	if err != nil {
		return nil, err
	}
	for _, path := range existing {
		if _, expected := generated[filepath.Base(path)]; expected {
			continue
		}
		...
		if strings.TrimSuffix(strings.SplitN(string(data), "\n", 2)[0], "\r") == "// Code generated by BSB. DO NOT EDIT." {
