...
export class SBFinder {
  public static getPluginType(name: string): IPluginDefinition {
  ...
  private static async findPluginsFiles(
    ...
    for (const dirPluginFolderName of readdirSync(path)) {
      ...
      arrOfPlugins.push({
        pluginDefinition: SBFinder.getPluginType(dirPluginFolderName),
  ...
  private static async findPluginsInBase(
    ...
    return await SBFinder.findPluginsFiles(
      coreLogger,
      innerPluginLibPlugin,
      packageVersion,
      libOnly
      ...
      try {
        ...
        if (dirFileWhat.indexOf("@") === 0) {
          ...
          for (const groupPluginName of readdirSync(pluginPath)) {
            ...
            if (statSync(groupPluginPath).isDirectory()) {
              arrOfPlugins = arrOfPlugins.concat(
                await SBFinder.findPluginsInBase(
                  coreLogger,
                  groupPluginPath,
                  true
                )
          ...
          if (statSync(pluginPath).isDirectory()) {
            arrOfPlugins = arrOfPlugins.concat(
              await SBFinder.findPluginsInBase(coreLogger, pluginPath, true)
            );
          }
        }
      } catch (err: any) {
  ...
  public static async findLocalPlugins(
    ...
    return await SBFinder.findPluginsInBase(coreLogger, cwd);
  }
}
