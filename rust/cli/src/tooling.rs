...
pub(crate) async fn install_schema(cwd: &Path, schema: &Value, name: &str) -> Result<Vec<PathBuf>> {
    ...
    atomic_write(&directory.join(format!("{name}.json")), data.as_bytes()).await?;
...
pub async fn build_host(cwd: &Path) -> Result<PathBuf> {
    ...
    let cli_source = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let mut cli = toml::map::Map::from_iter([
        (
            "package".into(),
            toml::Value::String("better-service-base-cli".into()),
        ),
        (
            "version".into(),
    ...
    if cli_source.join("Cargo.toml").is_file() {
        cli.insert(
            "path".into(),
            toml::Value::String(cli_source.to_string_lossy().into_owned()),
        );
    }
    linked.insert("bsb_cli".into(), toml::Value::Table(cli));
    ...
    source.push_str("if let Err(error)=runtime.block_on(bsb_cli::main_with_registry(registry)){eprintln!(\"BSB: {error:#}\");std::process::exit(1);}}\n");
...
pub async fn command(cwd: &Path, args: &[String]) -> Result<bool> {
    ...
    if action == "install" && positional.len() == 1 && Url::parse(positional[0]).is_ok() {
        let client =
            crate::hosted::HostedClient::new(positional[0], flags.contains_key("allow-insecure"))?;
        println!(
            "{}",
