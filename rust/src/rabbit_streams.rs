use crate::{
    events::{Reader, StreamHandler},
    observable::{Observable, Trace},
    rabbit::{Guard, Rabbit, Receiver, Sender},
};
...
impl Rabbit {
   ...
   if !called {if let Err(error)=&result {let _=tokio::time::timeout(Duration::from_secs(5),handler(obs.clone(),crate::events::failed_reader(error.to_string()))).await;}}
